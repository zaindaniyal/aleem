import sqlite3
table_name = "quran_verse_end_symbol"
...
def createTable(c):
    ...
    c.execute("CREATE TABLE " + table_name + "(id integer primary key, chapter_id INTEGER, verse_id INTEGER, arabic_text TEXT)")

def insertTranslation(c, file):
    for line in file.readlines():
        data = line.split()
        string = ' '.join (data)
        c.execute("INSERT INTO " + table_name + " (arabic_text) VALUES (:fullString)", {'fullString': string})
...
def readTextfile():
    file = open('QuranVerseEndSymbol.txt')
    return file
